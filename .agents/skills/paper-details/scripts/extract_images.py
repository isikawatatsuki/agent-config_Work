...
def extract(pdf_path, out_dir, dpi):
    if fitz is None:
        raise RuntimeError("PyMuPDFが必要です。使用するPython環境に pymupdf>=1.24.3 をインストールしてください。")
    if dpi <= 0:
        raise ValueError("--dpi は正の整数で指定してください。")
    with fitz.open(pdf_path) as doc:
        return extract_document(doc, pdf_path, out_dir, dpi)


def extract_document(doc, pdf_path, out_dir, dpi):
...
def main():
    ap = argparse.ArgumentParser(
        description="PDFの図表を画像として抽出します（Linux・Windows・macOS対応）。"
    )
    ap.add_argument("pdf", help="入力PDFのパス")
    ap.add_argument(
        "--out",
        default="images-from-papers",
        help="出力ディレクトリ（既定: ./images-from-papers）",
    )
    ap.add_argument("--dpi", type=int, default=300, help="画像の解像度（正の整数、既定: 300）")
    args = ap.parse_args()
